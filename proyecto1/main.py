...
def start():
    ...
    for _ in range(environment.CANT_SENSORES):
        thread = threading.Thread(
            ...
            args=(environment.SENSOR_TEMPERATURA, environment.ARCHIVO_TEMPERATURA),
        )
        thread.start()
    ...
    for _ in range(environment.CANT_SENSORES):
        thread = threading.Thread(
            target=create_sensor,
            args=(environment.SENSOR_HUMEDAD, environment.ARCHIVO_HUMEDAD),
        )
        thread.start()
    ...
    for _ in range(environment.CANT_SENSORES):
        thread = threading.Thread(
            target=create_sensor,
            args=(environment.SENSOR_HUMO, environment.ARCHIVO_HUMO),
        )
        thread.start()
...
def create_sensor(nombre, archivo):
    ...
    if nombre == environment.SENSOR_HUMO:
        sensor = SensorHumo(
            nombre, prob_correctos, prob_fuera_rango, prob_errores, contador
        )
